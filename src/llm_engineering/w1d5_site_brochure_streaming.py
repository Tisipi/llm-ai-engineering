...
def ask_llm_to_make_brochure_from_website(url):
    """Generate a brochure from the content at the given URL using streaming for real-time output."""
    print("Using OpenAI API to create a brochure (in streaming mode) from the website", url)
    ...
    for chunk in stream:
        # Extract the content from the chunk; cater for empty chunks
        content = chunk.choices[0].delta.content or ''
        # Print the content without a newline to create a continuous stream
...
def create_brochure_from_website(url):
    try:
        ask_llm_to_make_brochure_from_website(url)
